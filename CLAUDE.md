...
8. **Time is taint, not preface** (Chronon Plane, v2.5) — every persisted timestamp flows through an injected `Clock` (production: `SystemClock`; tests: `FrozenClock`; replay: `VirtualClock`); every externally-observed fact the model sees can carry `(observed_at, valid_at)`; contracts may bound wall-clock spend via `scope.max_wall_secs`; open turns emit `TurnHeartbeat` at a throttled cadence. Time never enters the constitution lane as a frozen string.
